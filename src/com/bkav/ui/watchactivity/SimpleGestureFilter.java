...
//		velocityX = Math.abs(velocityX);
		...
		// this.simpleGestureListener.onSwipe(SWIPE_LEFT);
		// else
		// this.simpleGestureListener.onSwipe(SWIPE_RIGHT);
		//
		// result = true;
		// }
		// else
		if (velocityY > this.swipe_Min_Velocity
			...
			if (e1.getY() > e2.getY()) {
				// bottom to up
				this.simpleGestureListener.onSwipe(SWIPE_UP);
			}
			else {
				this.simpleGestureListener.onSwipe(SWIPE_DOWN);
			}
			result = true;
		}
	...
	public boolean onDoubleTap(MotionEvent arg) {
		this.simpleGestureListener.onDoubleTap();
		return true;
	}
